      ...
      - generic [ref=e46]:
        ...
        - generic [ref=e51]:
          ...
          - generic [ref=e54]:
            ...
            - button "Change profile photo" [ref=e57]:
              - img [ref=e58]
          - generic [ref=e61]:
            - generic [ref=e62]:
              - text: Full Name
...
```ts
  ...
  31 | 
  32 |   // Upload IMG_8163 which causes "Something went wrong" error
  33 |   await fileInput.setInputFiles([path.join(resourcesDir, 'IMG_8163.png')]);
  34 |   await page.waitForLoadState('networkidle');
  35 | 
  36 |   // Save
  37 |   await settings.saveProfile();
  38 | 
  39 |   // Wait for the toast to appear (it's slow)
  40 |   await page.waitForTimeout(20000);
  41 | 
  42 |   // Log what's visible to debug
  43 |   const bodyText = await page.locator('body').innerText();
  44 |   const lines = bodyText.split('\n').filter(l => l.trim());
  45 |   console.log('=== Visible text after 20s ===');
  46 |   lines.forEach(l => console.log('  >', l.trim()));
  47 | 
> 48 |   await page.screenshot({ path: 'Resources/bug-evidence-settings.png', fullPage: true });
     |              ^ Error: page.screenshot: Test timeout of 30000ms exceeded.
  49 |   console.log('Screenshot saved');
  50 | });
  51 | 
```